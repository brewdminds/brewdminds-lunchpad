    ...
    <SiteLayout>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <div className="absolute -top-40 -right-24 h-[500px] w-[500px] rounded-full bg-[color:var(--cream)] blur-3xl opacity-70" />
          <div className="absolute top-40 -left-32 h-[400px] w-[400px] rounded-full bg-[color:var(--ember)]/10 blur-3xl" />
        </div>

        <div className="container-page pt-16 pb-24 md:pt-24 md:pb-32">
          <div className="max-w-4xl animate-rise">
            <p className="eyebrow">Social · Strategy · Craft</p>
            <h1 className="mt-5 text-5xl md:text-7xl font-semibold leading-[1.05] tracking-tight">
              Brands that show up with{" "}
              <span className="italic text-[color:var(--ember)]">intention</span>
              <span className="text-[color:var(--ink)]/40">.</span>
              <br />
              Not just content.
            </h1>
            <p className="mt-6 max-w-2xl text-lg md:text-xl text-[color:var(--ink)]/75">
              Brewdminds is a strategy-first, craft-obsessed social and marketing agency.
              We build brand systems, produce cinematic content, and win the next search
              paradigm — AEO — before your competitors know it exists.
            </p>
          ...
          {/* trust strip */}
          <div className="mt-20 border-t border-[color:var(--border)] pt-8">
            <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--ink)]/50 mb-6">
              Trusted by ambitious brands
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-8 items-center opacity-70">
              {["ATELIER", "NORTH&CO", "LUMEN", "SAVANT", "MERAKI", "OKTA STUDIOS"].map((n) => (
                <span key={n} className="text-sm md:text-base font-semibold tracking-widest text-[color:var(--ink)]/70 text-center">
                  {n}
                </span>
              ))}
            </div>
      ...
      <section className="container-page py-24">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div>
            ...
            <h2 className="mt-3 text-4xl md:text-5xl font-semibold">Nine pillars. One point of view.</h2>
          </div>
      ...
      <section className="container-page py-24">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 text-4xl md:text-5xl font-semibold">A process built for clarity.</h2>
      ...
      <section className="container-page pb-24">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="mt-3 text-4xl font-semibold">Sharp takes, not thought-leadership fluff.</h2>
          </div>
          <Link to="/blogs" className="hidden md:inline-flex btn-ghost">
          ...
          {posts.map((p) => (
            <article key={p.title} className="group rounded-2xl border border-[color:var(--border)] overflow-hidden bg-white hover:border-[color:var(--ember)] transition-colors">
              ...
              <div className="p-6">
                ...
                <h3 className="mt-3 text-lg font-semibold group-hover:text-[color:var(--ember)] transition-colors">{p.title}</h3>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-[color:var(--ink)] px-8 py-16 md:px-16 md:py-24 text-white">
          ...
          <div className="relative max-w-2xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link to="/services" className="inline-flex items-center gap-2 rounded-full border border-white/30 px-6 py-3 text-sm text-white hover:bg-white hover:text-[color:var(--ink)] transition-colors">
                See what we do
              </Link>
            </div>
          </div>
