...
/**
 * NOTE on the requested per-element animations: the "growth arrow
 * path-drawing" effect assumes it's a separate vector layer.
 * /public/brew-hero-final.png is a single flat RGB photo — the blocks and
 * arrow are baked into its pixels, so there's nothing to select and animate
 * independently. Everything else in the brief (Ken Burns zoom + parallax on
 * the whole image, staggered content entrance) is implemented below.
 */
...
export default function HeroPreviewPage() {
  ...
  return (
    <section
      className="relative isolate -mt-24 flex min-h-[calc(78vh+6rem)] items-center overflow-hidden pt-24"
    >
      {/* Background image — Ken Burns zoom + subtle mouse parallax + a soft reveal wipe on load.
          -mt-24/pt-24 above pulls the image up behind the (transparent-until-scroll) fixed nav,
          instead of leaving a gap of plain white between the nav and the photo. */}
      <motion.div
        aria-hidden="true"
      ...
      <div className="container-page">
        <div className="w-full max-w-xl lg:w-[42%]">
          ...
          <h1 className="mt-5 text-4xl leading-[1.05] tracking-tight sm:text-5xl lg:text-6xl">
          ...
          <motion.p
            ...
            className="mt-5 text-lg text-[color:var(--ink)]/75"
          >
